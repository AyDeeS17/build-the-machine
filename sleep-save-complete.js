...
const dayName=d=>dateFor(1,d).toLocaleDateString('en-US',{weekday:'long'});
...
function nutritionState(){
  for(const k of NUT_KEYS){const v=read(k,null);if(v&&typeof v==='object'&&Object.keys(v).length)return v;}
  return {};
}
...
function getCurrentDay(containerId){const c=$(containerId);if(!c)return 0;const a=[...c.children];const active=c.querySelector('.day.active');return Math.max(0,a.indexOf(active));}

function installRunningControls(){
  const view=$('btm-running-view');if(!view)return;
  const weeks=$('runWeeks'),days=$('runDays');
  patchWeekButtons(weeks,runningWeekDone);
  patchDayButtons(days,d=>runDone(getCurrentWeek('runWeeks'),d),[0,2,4]);
  const w=getCurrentWeek('runWeeks'),d=getCurrentDay('runDays');
  const panel=$('runSave')?.parentElement;if(!panel)return;
  panel.querySelector('#btmRunUnifiedControls')?.remove();
  const row=document.createElement('div');row.id='btmRunUnifiedControls';row.className='btm-complete-row';
  row.innerHTML='<button class="btm-unified-save">SAVE</button><button class="btm-unified-complete">COMPLETE</button>';
  panel.appendChild(row);
  row.querySelector('.btm-unified-save').onclick=()=>saveRunning(w,d);
  row.querySelector('.btm-unified-complete').onclick=()=>completeRunning(w,d);
  row.querySelector('.btm-unified-complete').classList.toggle('is-complete',runDone(w,d));
  const legacy=$('runSave');if(legacy)legacy.style.display='none';
  const legacyCheck=$('runDone')?.closest('.btm-check');if(legacyCheck)legacyCheck.style.display='none';
}

function installSleepControls(){
  const view=$('btm-sleep-view');if(!view)return;
  const weeks=$('sleepWeeks'),days=$('sleepDays');
  patchWeekButtons(weeks,sleepWeekDone);
  patchDayButtons(days,d=>sleepDone(getCurrentWeek('sleepWeeks'),d));
  const save=$('sleepSave');if(!save)return;
  const panel=save.parentElement;
  panel.querySelector('#btmSleepUnifiedControls')?.remove();
  const row=document.createElement('div');row.id='btmSleepUnifiedControls';row.className='btm-complete-row';
  row.innerHTML='<button class="btm-unified-save">SAVE</button><button class="btm-unified-complete">COMPLETE</button>';
  panel.appendChild(row);
  const w=getCurrentWeek('sleepWeeks'),d=getCurrentDay('sleepDays');
  row.querySelector('.btm-unified-save').onclick=()=>saveSleep(w,d);
  row.querySelector('.btm-unified-complete').onclick=()=>completeSleep(w,d);
  row.querySelector('.btm-unified-complete').classList.toggle('is-complete',sleepDone(w,d));
  save.style.display='none';
}

function installTrainingControls(){
  const grid=$('weekGrid');if(!grid)return;
  patchWeekButtons(grid,trainingDone);
  const toolbar=grid.parentElement; if(!toolbar)return;
  let panel=toolbar.querySelector('#btmTrainingWeekCompletePanel');
  if(!panel){panel=document.createElement('div');panel.id='btmTrainingWeekCompletePanel';panel.className='btm-week-complete-panel';panel.innerHTML='<span class="note" id="btmTrainingWeekStatus"></span><button class="btm-training-complete" id="btmTrainingCompleteBtn"></button>';toolbar.appendChild(panel);}
  const w=getCurrentWeek('weekGrid'),done=trainingDone(w);
  $('btmTrainingWeekStatus').textContent=done?'✓ WEEK '+w+' COMPLETED':'TRAINING WEEK '+w;
  const btn=$('btmTrainingCompleteBtn');btn.textContent=done?'✓ COMPLETED':'COMPLETE WEEK';btn.classList.toggle('is-complete',done);btn.onclick=()=>{setTrainingWeek(w,!trainingDone(w));toast(trainingDone(w)?'✓ TRAINING WEEK '+w+' COMPLETED':'TRAINING WEEK '+w+' REOPENED');installTrainingControls();};
}

function refreshSection(name){
  try{
    if(name==='running'&&typeof window.renderRunning==='function')window.renderRunning();
    if(name==='sleep'&&typeof window.renderSleep==='function')window.renderSleep();
    if(name==='food'&&typeof window.render==='function'&&window.__BTM_NUTRITION_MANUAL_V5__)window.render();
  }catch(e){}
  setTimeout(patchAll,30);
}
function patchAll(){
  installTrainingControls();
  installRunningControls();
  installSleepControls();
  patchNutrition();
}
function patchNutrition(){
  const view=$('btm-food-view');if(!view)return;
  const weeks=view.querySelector('#foodWeeks'),days=view.querySelector('#foodDays');
  const doneFn=(w)=>nutritionWeekDone(w);
  patchWeekButtons(weeks,doneFn);
  patchDayButtons(days,d=>nutritionDone(getCurrentWeek('foodWeeks'),d));
}

function observe(){
  const mo=new MutationObserver(()=>{clearTimeout(observe.t);observe.t=setTimeout(patchAll,20)});
  mo.observe(document.body,{childList:true,subtree:true});
  window.addEventListener('storage',()=>setTimeout(patchAll,30));
