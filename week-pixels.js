...
(()=>{
  ...
  const sprite=(c)=>{
    const s=[],skin=c.skin,hair=c.hair,outfit=c.outfit,a=c.accent;
  ...
  const apply=()=>{const grid=document.getElementById('weekGrid');if(!grid)return;[...grid.querySelectorAll('.week')].slice(0,12).forEach((card,i)=>{if(card.querySelector('.btm-week-pixel'))return;const c=CHARACTERS[i];const wrap=document.createElement('span');wrap.className='btm-week-pixel';wrap.dataset.character=c.name;wrap.title=c.name;wrap.setAttribute('aria-hidden','true');wrap.innerHTML=sprite(c);card.appendChild(wrap)})};
  const boot=()=>{install();apply();const watch=new MutationObserver(()=>{apply();const grid=document.getElementById('weekGrid');if(grid){new MutationObserver(apply).observe(grid,{childList:true,subtree:true});watch.disconnect()}});watch.observe(document.body,{childList:true,subtree:true});setTimeout(()=>{apply();watch.disconnect()},8000)};
